...
public class Receive_acct_Service_Impl extends BaseService implements Receive_acct_Service_Iface {
	...
	public List show_receive_acct(Receive_acct receive_acct) throws Exception{
		...
		if(zfb_ls.size()>0){
			...
			t_lz_zfb_info.setApp_id(zfb_ls.get(0).getApp_id());
			t_lz_zfb_info.setMerchant_private_key(zfb_ls.get(0).getMerchant_private_key());
			t_lz_zfb_info.setAlipay_public_key(zfb_ls.get(0).getAlipay_public_key());
			t_lz_zfb_info.setNotify_url(zfb_ls.get(0).getNotify_url());
			t_lz_zfb_info.setReturn_url(zfb_ls.get(0).getReturn_url());
			t_lz_zfb_info.setSign_type(zfb_ls.get(0).getSign_type());
			t_lz_zfb_info.setCharset(zfb_ls.get(0).getCharset());
			t_lz_zfb_info.setGatewayurl(zfb_ls.get(0).getGatewayurl());
			t_lz_zfb_info.setZfbchar(zfb_ls.get(0).getZfbchar());
		}
		receive_acct.setCheck(check);
		ls.add(t_lz_zfb_info);
		return ls;
	}
}
